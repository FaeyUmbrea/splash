...
/**
 * Presets are `splash.preset` JournalEntryPage subtypes in one lazily-created world `JournalEntry`.
 * All host coupling is isolated to this file.
 */
...
export interface PresetSummary {
	...
	kind: PresetKind;
	img: string;
	/** PrefabBehavior key, set when the preset reconfigures via a dialog on apply. */
	behavior?: string | null;
}

/** The payload as stored on a preset page's `system.payload`. */
...
// Core glyphs stand in for kinds with no natural thumbnail, avoiding any shipped art dependency in v1.
...
/** Presets carry style and content only; every onClick collapses to a bare close. */
...
/** Style, label, and image set carry over; the action is stripped. */
...
/** Strips world-specific placement (`states:{}`) so only content and style travel into the stored preset. */
...
/** Re-sanitizes the action: an imported preset never went through our save, so don't trust it. */
...
/** Unlike a single style preset, a group prefab keeps its placement and onClick; the editor remaps ids on apply. */
...
/** Single sprites are stripped to style; groups keep their layout. */
...
/** Looks up the library journal by name; does not create it. */
export function findPresetLibrary(): JournalEntry | undefined {
	return game.journal?.getName(PRESET_LIBRARY_NAME) ?? undefined;
}

/** GM-only. Concurrent calls share one in-flight create so two rapid saves never race two journals. */
...
/**
 * Lets a module ship presets that need no import. The pack index already lists each entry's page types,
 * so only entries that actually contain `splash.preset` pages get loaded, never whole unrelated lore packs.
 */
...
/** GM-only, lazily creating the library. Create-only in v1, no overwrite. */
...
/**
 * Reads source data via `system.toObject()`, not initialized `system.payload`, because the editor writes
 * source data (a ColorField tint must be a hex string, not a live Color instance).
 */
...
/** A no-op, with a warning, for players. */
